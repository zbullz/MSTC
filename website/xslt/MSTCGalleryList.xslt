...
<xsl:template match="/">
  ...
  <xsl:for-each select="umbraco.library:GetMedia($galleryRootId, 1)/Folder">
  ...
  <div class="gallery_link">
       <label>
             <a href="{$galleryDisplayPage}?galleryId={$gallery/@id}">
               <xsl:value-of select="$gallery/@nodeName"/></a></label><br />
    <a href="{$galleryDisplayPage}?galleryId={$gallery/@id}">
      <xsl:for-each select="$gallery/Image[position() &lt;= 4]">
        <img class="pos{position()}" src="/umbraco/ImageGen.ashx?width=70&amp;height=50&amp;crop=1&amp;image={umbracoFile}"/>
      </xsl:for-each>
    </a>
  </div>
</xsl:template>
